...
router.post("/save", ensureAuthenticatedInnerRoutes, async (req, res) => {
  try {
    ...
    if (formCount >= 5) {
      return res
        .status(400)
        .json({ error: "You can only create up to 5 forms." });
    }
    ...
    if (!name || typeof name !== "string") {
      return res
        .status(400)
        .json({ error: "Form name is required and must be a string." });
    }
    ...
    if (!Array.isArray(components) || components.length === 0) {
      return res
        .status(400)
        .json({ error: "Form must include at least one component." });
    }

    // Validate components
    for (const component of components) {
      if (
        !["textbox", "star", "combobox", "title", "scale-bar"].includes(
          component.type,
        )
      ) {
        return res
      ...
      if (!component.text || typeof component.text !== "string") {
        return res
          .status(400)
          .json({ error: "Each component must have a text field." });
      }
      if (component.type === "combobox") {
        if (
          ...
          return res.status(400).json({
            error:
              "ComboBox components must include valid options with a numeric order.",
          });
        }
      }
      ...
      if (theme === "custom" && component.colors) {
        if (
          typeof component.colors !== "object" ||
          Array.isArray(component.colors)
        ) {
          return res
            .status(400)
            .json({ error: "Colors must be a valid map (object)." });
        }
      }
    }
    console.log({
...
router.get("/:id", validateApiKey, async (req, res) => {
  try {
    ...
    if (!form) {
      return res
        .status(403)
        .json({ error: "Form not found  or Unauthorized access." });
    }
...
router.delete("/:id", ensureAuthenticatedInnerRoutes, async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Form and associated feedback deleted successfully." });
  } catch (error) {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.log(
        "number of request the user made in 24 hours are " + requestCount,
      );
      if (requestCount >= 2) {
        return res.status(429).json({
          success: false,
          error: "You can only generate a form twice per day.",
        });
      }
...
`,
      ...
      formJson.components.forEach((component) => {
        ...
        if (!component.text || typeof component.text !== "string") {
          return res
            .status(400)
            .json({ error: "Each component must have a valid text field." });
        }
        ...
        if (theme === "custom" && component.colors) {
          if (
            ...
            return res.status(400).json({
              error: "Colors must be a valid object for custom themes.",
            });
          }
        }
      });
      ...
      formObject.components = formObject.components.map((component) => {
        ...
        if (component.additionalData instanceof Map) {
          component.additionalData = Object.fromEntries(
            component.additionalData,
          );
        }
        return component;
...
router.post("/send-email/:id", async (req, res) => {
  ...
  if (!email || !htmlContent || !formId) {
    return res
      .status(400)
      .json({ error: "Email, HTML content, and Form ID are required." });
  }

  try {
    // Read the CSS file
    const cssPathBuilder = path.join(
      __dirname,
      "..",
      "public",
      "stylesheets",
